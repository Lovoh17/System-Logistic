...
class TransportistaResource extends Resource
{
            ...
            Forms\Components\Section::make('Vehículo')
                ...
                ->schema([
                    ...
                    Forms\Components\TextInput::make('capacidad_kg')
                        ->label('Capacidad Peso (kg)')->numeric()->columnSpan(1),

                    Forms\Components\TextInput::make('capacidad_m3')
                        ->label('Capacidad Volumen (m³)')->numeric()->columnSpan(1),

                    Forms\Components\Toggle::make('tiene_refrigeracion')
                        ->label('¿Tiene Refrigeración?')->columnSpan(1),

                    Forms\Components\Toggle::make('tiene_gps')
                        ->label('¿Tiene GPS?')->columnSpan(1),
                ]),
            ...
            Forms\Components\Section::make('Conductor')
                ...
                ->schema([
                    Forms\Components\TextInput::make('conductor_nombre')
                        ->label('Nombre del Conductor')->maxLength(100)->columnSpan(1),
                    Forms\Components\TextInput::make('conductor_licencia')
                        ->label('N° Licencia')->maxLength(30)->columnSpan(1),
                    Forms\Components\TextInput::make('conductor_telefono')
                        ->label('Teléfono del Conductor')->maxLength(20)->columnSpan(1),
                ]),

            Forms\Components\Section::make('Tarifas')
                ->columns(3)
                ->schema([
                    Forms\Components\TextInput::make('tarifa_km')
                        ->label('Tarifa por Km ($)')->numeric()->prefix('$')->step(0.01)->columnSpan(1),
                    Forms\Components\TextInput::make('tarifa_fija')
                        ->label('Tarifa Fija ($)')->numeric()->prefix('$')->step(0.01)->columnSpan(1),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('codigo')->badge()->color('gray'),
                Tables\Columns\TextColumn::make('nombre')->searchable()->sortable()
                    ->description(fn($record) => ucfirst($record->tipo)),
                Tables\Columns\TextColumn::make('vehiculo_tipo')->label('Vehículo')
                    ->badge()->color('info'),
                Tables\Columns\TextColumn::make('vehiculo_placa')->label('Placa'),
                Tables\Columns\TextColumn::make('conductor_nombre')->label('Conductor')
                ...
                Tables\Columns\TextColumn::make('capacidad_kg')->label('Cap. kg')->toggleable(),
