        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <x-filament::section>
                    ...
                    <input
                            ...
                            placeholder="Nombre, codigo o SKU del producto..."
                        ...
                        <div class="mt-3">
                                        ...
                                        <div wire:click="agregarProducto({{ $item->producto_id ?? $item->id }})"
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-xs text-gray-400 mt-0.5">
                                                    ...
                                                    <span @class([
                                                    'font-semibold',
                                                    'text-danger-500'                    => ($item->stock_actual) <= 5,
                                                    'text-success-600 dark:text-success-400' => ($item->stock_actual) > 5,
                                                ])>{{ $item->stock_actual }}</span>
                                                </p>
                                            </div>
                                            <span class="font-bold text-primary-600 dark:text-primary-400 text-sm ml-3 flex-shrink-0">
                            ...
                            @else
                                <div class="text-center py-8 text-gray-400">
                                    <x-heroicon-m-face-frown class="w-10 h-10 mx-auto mb-2 opacity-30"/>
                ...
                @if(empty($searchTerm) || strlen($searchTerm) < 2)
                    <x-filament::section>
                        ...
                        @if($this->productosRapidos->isEmpty())
                            <div class="text-center py-8 text-gray-400">
                                <x-heroicon-m-archive-box-x-mark class="w-10 h-10 mx-auto mb-2 opacity-30"/>
                                ...
                                @foreach($this->productosRapidos as $item)
                                    ...
                                    >
                                        <p class="font-semibold text-sm text-gray-800 dark:text-gray-200
                                              leading-tight group-hover:text-primary-700 dark:group-hover:text-primary-400">
            ...
            <div class="space-y-4">
                ...
                <x-filament::section>
                    <x-slot name="heading">
                    <span class="flex items-center gap-2">
                        <x-heroicon-m-user class="w-4 h-4 text-info-500"/>
                        Cliente
                    </span>
                    </x-slot>
                ...
                <x-filament::section class="sticky top-4">
                    ...
                    <span class="flex items-center gap-2">
                        ...
                        @if($this->itemsCount > 0)
                            <span class="bg-primary-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                                {{ $this->itemsCount }}
                            </span>
                        @endif
                    </span>
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-gray-700 max-h-80 overflow-y-auto -mx-1 px-1">
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        <button wire:click="actualizarCantidad({{ $item['id'] }}, {{ $item['cantidad'] - 1 }})"
                                                class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700
                                                   ...
                                                   text-gray-700 dark:text-gray-200 text-sm font-bold transition">
                                            −
                                        </button>
                                        <span class="w-7 text-center text-sm font-semibold text-gray-800 dark:text-gray-200">
                                        {{ $item['cantidad'] }}
                                    </span>
                                        <button wire:click="actualizarCantidad({{ $item['id'] }}, {{ $item['cantidad'] + 1 }})"
                                                class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700
                        ...
                        @empty
                            <div class="py-10 text-center text-gray-400">
                                ...
                                <p class="text-sm font-medium">Carrito vacio</p>
                                <p class="text-xs mt-1 opacity-75">Agrega productos para comenzar</p>
                            </div>
                        @endforelse
                    </div>
                    ...
                    @elseif(!$cliente_id)
                        <p class="text-xs text-center text-warning-500 dark:text-warning-400 mt-2">
