    ...
    <div class="space-y-4">
        ...
        <div class="flex gap-4">
            ...
            <div class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-5 h-24">
                <div class="flex items-center justify-between h-full">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Disponibles</p>
            ...
            <div class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-5 h-24">
                <div class="flex items-center justify-between h-full">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">En Ruta</p>
            ...
            <div class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-5 h-24">
                <div class="flex items-center justify-between h-full">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Mantenimiento</p>
        ...
        <x-filament::section>
            <div id="map" style="height: 520px; width: 100%; border-radius: 8px; position: relative; z-index: 1;"></div>
        </x-filament::section>
                                ...
                                <div class="min-w-0">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-2 space-y-0.5">
                                        ...
                                        <p class="flex items-center gap-1">
                                            ...
                                            {{ $transportista->conductor_telefono ?? $transportista->telefono ?? 'Sin telefono' }}
                                        </p>
                                    </div>
                                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tr class="border-b border-gray-200 dark:border-gray-700">
                        <th class="text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Nombre</th>
                        <th class="text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Vehiculo</th>
                        <th class="text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Conductor</th>
                        <th class="text-center py-2.5 px-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Estado</th>
                        <th class="text-center py-2.5 px-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Ubicacion</th>
                    </tr>
                    </thead>
...
@push('styles')
    ...
    <style>
        ...
        .tl-marker-icon { transform: rotate(45deg); display: flex; align-items: center; justify-content: center; }
    </style>
@endpush
...
@push('scripts')
    ...
    <script>
        ...
        function initMap() {
            // Limpiar instancia anterior si existe (navegación Livewire)
        ...
        function cargarMarcadores() {
            ...
            @json($this->todosTransportistas).forEach(function(t) {
                ...
                <div style="min-width:210px;font-size:13px;">
                    <p style="font-weight:700;font-size:14px;border-bottom:1px solid #eee;padding-bottom:6px;margin-bottom:8px;">${t.nombre}</p>
                    <p style="color:#555;margin:3px 0;"><strong>Vehiculo:</strong> ${t.vehiculo_tipo ?? '—'} (${t.vehiculo_placa ?? '—'})</p>
                    <p style="color:#555;margin:3px 0;"><strong>Conductor:</strong> ${t.conductor_nombre ?? 'N/A'}</p>
                    <p style="color:#555;margin:3px 0;"><strong>Tel:</strong> ${t.conductor_telefono ?? t.telefono ?? 'N/A'}</p>
                    <p style="color:#555;margin:3px 0;"><strong>Ubicacion:</strong> ${t.ubicacion_actual ?? `${(+t.latitud).toFixed(4)}, ${(+t.longitud).toFixed(4)}`}</p>
                    <p style="margin-top:8px;">
                        <span style="background:${color};color:#fff;padding:2px 8px;border-radius:999px;font-size:11px;font-weight:600;">
        ...
        // Inicializar en DOMContentLoaded, livewire:navigated y como fallback
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initMap);
        } else {
            // DOM ya listo (ej. navegación SPA de Filament)
            setTimeout(initMap, 50);
        }
        document.addEventListener('livewire:navigated', function () { initMap(); });
