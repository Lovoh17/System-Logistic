<x-filament-panels::page>
    ...
    @php
        ...
        $centroLat = $tieneMapa ? ($origenLat + $destinoLat) / 2 : ($tieneOrigen ? $origenLat : 13.794);
        $centroLng = $tieneMapa ? ($origenLng + $destinoLng) / 2 : ($tieneOrigen ? $origenLng : -88.896);
            ...
            <div class="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-5 h-24">
                <div class="flex items-center justify-between h-full">
                    <div>
                        ...
                        <p class="text-lg font-bold mt-0.5 text-{{ $estadoColor }}-600 dark:text-{{ $estadoColor }}-400">
                            {{ $estadoLabel }}
                        </p>
                    </div>
                    ...
                    @if($conductor)
                        <div class="space-y-2">
                            ...
                            <div class="border-t border-gray-100 dark:border-gray-700 pt-2 grid grid-cols-2 gap-1 text-xs">
                                ...
                                <span class="text-gray-700 dark:text-gray-300">{{ $transportista->tiene_refrigeracion ? 'Si' : '—' }}</span>
                            </div>
                            @if($tieneGps)
                                <p class="text-xs text-gray-400 pt-1">
                                    Ultima pos: {{ $transLat }}, {{ $transLng }}
                                </p>
                            @endif
                        </div>
                    @else
                ...
                <table class="w-full text-sm">
                    ...
                    @forelse($traslado->items as $item)
                        <tr class="hover:bg-gray-100/60 dark:hover:bg-white/5 transition-colors">
                            ...
                            <td class="py-3 px-3 text-right font-semibold text-primary-600 dark:text-primary-400 tabular-nums">
                                {{ number_format($item->cantidad_sugerida, 2) }}
                            </td>
            ...
            <script>
                (function () {
                    function initMapa() {
                        ...
                        var map = L.map('mapa-traslado').setView([{{ $centroLat }}, {{ $centroLng }}], 8);
                        ...
                        var iconA = L.divIcon({
                            className: '',
                            html: '<div style="background:#22c55e;color:#fff;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;box-shadow:0 2px 6px rgba(0,0,0,.35);">A</div>',
                            iconSize: [32,32], iconAnchor: [16,16],
                        });
                        var iconB = L.divIcon({
                            className: '',
                            html: '<div style="background:#ef4444;color:#fff;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;box-shadow:0 2px 6px rgba(0,0,0,.35);">B</div>',
                            iconSize: [32,32], iconAnchor: [16,16],
                        });

                        L.marker([{{ $origenLat }}, {{ $origenLng }}], {icon: iconA})
                            .bindPopup('<b>{{ addslashes($origen->nombre) }}</b>{{ $origen->direccion ? "<br>" . addslashes($origen->direccion) : "" }}')
                            .addTo(map);
                        L.marker([{{ $destinoLat }}, {{ $destinoLng }}], {icon: iconB})
                            .bindPopup('<b>{{ addslashes($destino->nombre) }}</b>{{ $destino->direccion ? "<br>" . addslashes($destino->direccion) : "" }}')
                            .addTo(map);
                        ...
                        @if($tieneGps)
                        var iconTruck = L.divIcon({
                            className: '',
                            html: '<div style="background:#f59e0b;color:#fff;border-radius:6px;padding:3px 6px;font-size:11px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,.35);">Truck</div>',
                            iconSize: [44, 28], iconAnchor: [22, 14],
                        });
                        L.marker([{{ $transLat }}, {{ $transLng }}], {icon: iconTruck})
                            .bindPopup('<b>{{ addslashes($conductor?->name ?? "Conductor") }}</b><br>{{ addslashes($transportista->vehiculo_placa ?? "") }}')
                            .addTo(map);
                        @endif
                    ...
                    document.addEventListener('livewire:navigated', initMapa);
                })();
            </script>
        @endpush
    @endif
