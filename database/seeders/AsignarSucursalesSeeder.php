    ...
    public function run(): void
    {
        ...
        foreach ($users as $user) {
            ...
            if ($codigoSucursal && isset($sucursales[$codigoSucursal])) {
                ...
                $this->command->warn("{$user->email} → No se encontró sucursal");
            }
            ...
            if ($user->roles->isEmpty()) {
                ...
                if ($rolAsignado) {
                    ...
                    if ($role) {
                        ...
                        $this->command->line("Rol asignado: {$rolAsignado}");
                    } else {
                        $this->command->warn("Rol '{$rolAsignado}' no existe. ¿Corriste DatabaseSeeder primero?");
                    }
                }
            } else {
                $this->command->line("Ya tiene rol: {$user->roles->pluck('name')->join(', ')}");
            }
        }
